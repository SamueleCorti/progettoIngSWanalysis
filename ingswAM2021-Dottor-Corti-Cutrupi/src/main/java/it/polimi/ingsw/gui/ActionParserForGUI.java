 ...
 */
public class ActionParserForGUI {
    private GuiSideSocket guiSideSocket;

    public ActionParserForGUI(GuiSideSocket guiSideSocket) {
    ...
    public Action parseInput(@NotNull String input){
        ...
        switch(command.toLowerCase()){
            ...
            case "wtcchoice":{
                ...
                int numOfBlanks = guiSideSocket.getNumOfBlanks();
                ...
                try {
                    for (int i=1; i < numOfBlanks+1; i++) {
                        ...
                        resources.add(type);
                        if (type<0 || type>guiSideSocket.getLeaderCardsKept()-1) {
                            isCorrect = false;
                        }
                    }
                    if (isCorrect) actionToSend = new WhiteToColorAction(resources);
                        ...
                        actionToSend = null;
                        System.out.println("Indexes must be between 1 and "+guiSideSocket.getLeaderCardsKept());
                    }
                }catch (IndexOutOfBoundsException e){
                    actionToSend = null;
